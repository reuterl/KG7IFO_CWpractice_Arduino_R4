...
ping::ping() {
	Length = 8;
	msg = new uint8_t[Length];
	initMsg(msg, Length, CmmdCode::cmmdPing);
	payload = 0;
}
ping::~ping() {}

void ping::rcvPing(uint8_t *_msg) {
	payload = MsgTo16(_msg, 4);
	Encode16(msg, 4, payload);
	CheckSum(msg, Length);
}

// send back verbatum
uint8_t *ping::echoPing(void) {
	return msg;
}

void ping::newPing(uint16_t _payload) {
	payload = _payload;
	Encode16(msg, 4, payload);
	CheckSum(msg, Length);
}
